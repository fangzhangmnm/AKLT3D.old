...
class AKLT3D(TNModel):
    ...
    def get_observable(self,name):
        if name=='magnetizationX':
            return self.get_ST0(axis=0),False
        if name=='magnetizationY':
            return self.get_ST0(axis=1),False
        if name=='magnetizationZ':
            return self.get_ST0(axis=2),True
    def get_observable_moments(self,name,n):
        if name=='magnetizationX':
            return [self.get_ST0(axis=0,moment=i) for i in range(n+1)],False
        if name=='magnetizationY':
            return [self.get_ST0(axis=1,moment=i) for i in range(n+1)],False
        if name=='magnetizationZ':
            return [self.get_ST0(axis=2,moment=i) for i in range(n+1)],True
    def get_ST0(self,axis,moment=1):
        return self.get_T(torch.linalg.matrix_power(get_Lxyz(j=3)[axis],moment))
    def get_T(self,op):
